...
func (db *Postgres) NewHiveWeight(ctx context.Context, weight http.HiveWeight) error {
...
func (db *Postgres) DeleteHiveWeight(ctx context.Context, weight http.HiveWeight) error {
...
func (db *Postgres) getHiveWeightsSinceTime(ctx context.Context, hive http.Hive, time time.Time) ([]http.HiveWeight, error) {
	...
	defer rows.Close()
	var weights []http.HiveWeight
	for rows.Next() {
		var weight http.HiveWeight
...
func (db *Postgres) GetHiveWeightForDay(ctx context.Context, hive http.Hive) ([]http.HiveWeight, error) {
	dayAgo := time.Now().Add(-24 * time.Hour)
	return db.getHiveWeightsSinceTime(ctx, hive, dayAgo)
}

func (db *Postgres) GetHiveWeightForWeek(ctx context.Context, hive http.Hive) ([]http.HiveWeight, error) {
	weekAgo := time.Now().Add(-7 * 24 * time.Hour)
	return db.getHiveWeightsSinceTime(ctx, hive, weekAgo)
}

func (db *Postgres) GetHiveWeightForMonth(ctx context.Context, hive http.Hive) ([]http.HiveWeight, error) {
