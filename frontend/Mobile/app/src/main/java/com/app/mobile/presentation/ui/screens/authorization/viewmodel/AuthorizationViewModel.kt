...
class AuthorizationViewModel(
...
) : BaseViewModel<AuthorizationUiState, AuthorizationEvent>(AuthorizationUiState.Loading) {
			...
			launch {
				when (val result = authorizationAccountUseCase(model.toDomain()).toAuthorizationUiModel()) {
					...
					is AuthorizationResultUi.Error   -> {
						updateState { AuthorizationUiState.Content(model, validatedFormState) }
						sendEvent(AuthorizationEvent.ShowSnackBar(result.message))
					}
				}
			}
		}
	...
	fun onRegistrationClick() {
		...
		if (state is AuthorizationUiState.Content) {
			launch {
				sendEvent(AuthorizationEvent.NavigateToRegistration)
			}
		}
	}
}