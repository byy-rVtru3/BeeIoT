...
Поднимается автоматически в составе `docker compose` стек-а как сервис `web-admin`. Главный nginx проксирует на неё корневой путь, поэтому админка доступна **по тому же `http://<host>/`**, что и REST API; отдельный порт `:3000` тоже открыт — удобно для прямого доступа в обход rate-limit при отладке. Логин разрешён только пользователям с флагом `is_admin = true` — назначается через `make admin EMAIL=...`.
