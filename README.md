...
2. Создать файл `backend/.env` с переменными (минимальный список — см. `build/docker-compose.yml`):
3. Поднять стек:
...
1. В `config.py` задать `DEVICE_ID`, адрес MQTT-брокера и параметры NB-IoT.
...
Поднимается автоматически в составе `docker compose` стек-а как сервис `web-admin`. Главный nginx проксирует на неё корневой путь, поэтому админка доступна **по тому же `http://<host>/`**, что и REST API.
...
* Календарь матки: автоматический расчёт фаз развития.
* Графики телеметрии температуры, шума и массы за произвольный период.
